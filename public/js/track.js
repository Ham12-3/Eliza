...
class TrackModel {
  ...
  posAt(corridor, arc) {
    ...
    // bearing: compass degrees clockwise from north
  ...
  // in increasing arc order (i.e. the direction of travel). Returns
  // { corridor, arcNext } or null.
    ...
    for (const c of this.corridors) {
      ...
      if (afterId != null && c.stationArc.has(afterId)) {
        if (c.stationArc.get(afterId) > aNext) {
          directional = true;
          score += 2;
        } else continue; // wrong direction on this corridor
      }
