...
//      through the corridor's stations. Also emit a speed (m/s) so the renderer
//      can dead-reckon continuous motion between polls.
...
// Empirical segment-time table (seconds), keyed undirected by station pair.
export function deriveSegmentTimes(trains, model) {
  ...
  const adjacent = (a, b) =>
    model.corridors.some((c) => {
      ...
      // adjacent if no other station's arc lies strictly between them
...
// Produce placed trains on the real track.
// Each: { id, corridorId, arc, lat, lon, bearing, speed, nextId, nextName,
//         eta, destinationName, direction }
export function placeTrains(trains, model, segTimes) {
  ...
  for (const [key, preds] of trains) {
    ...
    // Walk backward through corridor stations, consuming segment times, to find
    // the arc-length the train currently occupies and its current speed.
    ...
    placed.push({
      ...
      lon: pos.lon,
      bearing: pos.bearing,
      speed, // m/s along the corridor (always increasing arc)
...
// Convert "eta seconds before reaching `nextId` (at arcNext)" into an arc-length
// position, walking back segment by segment. Returns { arc, speed }.
function locateArc(model, corridor, segTimes, arcNext, nextId, eta) {
  ...
  for (let hop = 0; hop < 40; hop++) {
    ...
    if (!prev) break; // origin of the corridor
  ...
  // Couldn't walk far enough back — sit at the furthest station we reached.
